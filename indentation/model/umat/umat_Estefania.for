...
C
C --------------------------------------------------------------------------
C Subrutina UMAT para material hiperelastico anisotropo inelastico (dano y viscoelasticidad)
C Noviembre 2005
C GEMM-Unizar
C --------------------------------------------------------------------------
C
...
C ------------------------------------------------------------------------------
      ...
      elseif (abs(de(1)-11.0) <= 1.0E-6) then  
            call W_Aliliche(nprop,de,inva,aneurg,w,dw1,dw2)
      endif
            end subroutine derW
C ------------------------------------------------------------------------------
...
C ------------------------------------------------------------------------------
      subroutine W_Aliliche(nprop,de,inva,dam,w,dw1,dw2)
C ------------------------------------------------------------------------------
C
C Evaluates the HGO-Yeoh Strain energy density function for based on Aliliche, 2022
C
C------------------------------------------------------------------
C
C de      ... array of material properties
...
C------------------------------------------------------------------
      ...
      real*8, intent(out)   :: w(4),dw1(4),dw2(10)
      real*8                :: C10,C01,C20,C11,C02
      real*8                :: I1,I3,I4,I1b, I4b,stretcha,stretchb
      real*8                :: k1,k2,k3,k4,rho
      real*8                :: term1,term4,term6,exp1,exp2
      real*8                :: expon11,expon14,expon21,expon26
      real*8                :: a1, a2, a3, c1, c2, d
C
C Material properties
C
      d = de(2) ; 
      a1 = de(3) ; a2 = de(4) ; a3 = de(5) ; c1 = de(6) ; c2 = de(7)
      I1=inva(1); I3 = inva(3); I4=inva(4)
      I1b = matmul(I1, I3)
C
C Material properties
C
...
C Strain energy density function and derivatives
            w(2)= a1*(I1-3.0)+C01*(I2-3.0)+C20*(I1-3.0)**2  
            w(2)= w(2)+C11*(I1-3.0)*(I2-3.0)+C02*(I2-3.0)**2 
            w(3)= (k1/k2)*(exp1-1.0)
            w(4)= (k3/k4)*(exp2-1.0)
            w(1)= w(2) + w(3)+ w(4)
...
subroutine deftensorspolyconvex(FG,tenb,tenb2,inva)
      C ------------------------------------------------------------------------------
      C Calculates invariants and deformation tensors for the polyconvex law
      C ------------------------------------------------------------------------------
            INCLUDE 'aba_param.inc'
      
            real*8, intent(in)  :: FG(3,3)
            real*8, intent(out) :: tenb(6,1)
            real*8, intent(out) :: tenb2(6,1),invapolyconvex(6)
            real*8              :: tenb1(3,3), tenb12(3,3)
            real*8              :: tenC(3,3), vtemp(3,1)
            real*8              :: traceb2
            integer             :: i
      ...
      C Invariants
      C
            invapolyconvex(1)=sum((/(tenb1(i,i),i=1,3)/))
            traceb2=sum((/(tenb12(i,i),i=1,3)/))
            invapolyconvex(2)=0.5*(invapolyconvex(1)*invapolyconvex(1)-traceb2)
            vtemp=matmul(tenC,na0)
            invapolyconvex(3)=dot_product(na0(:,1),vtemp(:,1));
            vtemp=matmul(tenC,nb0)
            invapolyconvex(4)=dot_product(nb0(:,1),vtemp(:,1));
      C
      C Deformation tensors
      C
            tenb(1:3,1)=(/(tenb1(i,i),i=1,3)/)
            ...
            tenb2(1:3,1)=(/(tenb12(i,i),i=1,3)/)
            tenb2(4:6,1)=(/tenb12(1,2),tenb12(1,3),tenb12(2,3)/)
            end subroutine deftensorspolyconvex
      C
