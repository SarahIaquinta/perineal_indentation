...
def compute_stress_vector_step_variable_c1_relaxation(parameters, datafile, sheet, step, load_step_values):
...
def find_parameters_variable_c1_relaxation(datafile, sheet, step, previous_step_values, previous_step_optimized_parameters, minimization_method):
    _, _, _, _, _, _, _, index_init_step_dict_relaxation, index_final_step_dict_relaxation, elongation_init_step_dict_relaxation, elongation_final_step_dict_relaxation, elongation_list_during_steps_dict_relaxation, time_list_during_steps_dict_relaxation, stress_list_during_steps_dict_relaxation = extract_step_data(datafile, sheet)
    elongation_list, time, experimental_stress = elongation_list_during_steps_dict_relaxation[step], time_list_during_steps_dict_relaxation[step], stress_list_during_steps_dict_relaxation[step]

    def minimization_function(parameters):
        Q_list, S_list, S_H_list, Pi_list = compute_stress_vector_step_variable_c1_relaxation(parameters, datafile, sheet, step, previous_step_values)
        least_square = mean_squared_error(experimental_stress, Pi_list)
        return least_square

    res = minimize(minimization_function, x0=previous_step_optimized_parameters, method=minimization_method, bounds=[(0.1, 100)],tol=1e-2,
               options={'disp': False}) 
    # print(res.message)
    parameters = res.x
...
def find_parameters_all_steps_variable_c1(datafile, sheet, minimization_method_load, minimization_method_relaxation):
    c1_dict_load = {}
    beta_dict_load = {}
    tau_dict_load = {}
    fitted_stress_dict_load = {}
    ...
    initial_guess_load = [20, 1, 30]
    optimized_c1_step_0, optimized_beta_step_0, optimized_tau_step_0, parameters_step_0 = find_parameters_variable_c1_load(datafile, sheet, 0, previous_step_values, initial_guess_load, minimization_method_load)
    c1_dict_load[0] = optimized_c1_step_0
    beta_dict_load[0] = optimized_beta_step_0
    tau_dict_load[0] = optimized_tau_step_0
    ...
    optimized_tau_relaxation, parameters = find_parameters_variable_c1_relaxation(datafile, sheet, 0, load_step_values, previous_step_optimized_parameters_relaxation, minimization_method_relaxation)
    c1_dict_relaxation[0] = optimized_c1_step_0
    beta_dict_relaxation[0] = optimized_beta_step_0
    tau_dict_relaxation[0] = optimized_tau_relaxation
    Q_list, S_list, S_H_list, Pi_list = compute_stress_vector_step_variable_c1_relaxation(parameters,  datafile, sheet, 0, load_step_values)
    fitted_stress_dict_relaxation[0] = Pi_list
    previous_step_values = Q_list[-1], S_list[-1], S_H_list[-1], Pi_list[-1]
    previous_step_optimized_parameters_load = [optimized_c1_step_0, optimized_beta_step_0, optimized_tau_relaxation]
    previous_step_optimized_parameters_relaxation = [optimized_tau_relaxation]

    for p in list(step_numbers)[1:]:
        optimized_c1_load, optimized_beta_load, optimized_tau_load, parameters = find_parameters_variable_c1_load(datafile, sheet, p, previous_step_values, previous_step_optimized_parameters_load, minimization_method_load)
        ...
        previous_step_values = Q_list[-1], S_list[-1], S_H_list[-1], Pi_list[-1]
        load_step_values = previous_step_values
        previous_step_optimized_parameters_load = [optimized_c1_load, optimized_beta_load, optimized_tau_load]

        optimized_tau_relaxation, parameters = find_parameters_variable_c1_relaxation(datafile, sheet, p, load_step_values, previous_step_optimized_parameters_relaxation, minimization_method_relaxation)
        c1_dict_relaxation[p] = optimized_c1_load
        beta_dict_relaxation[p] = optimized_beta_load
        tau_dict_relaxation[p] = optimized_tau_relaxation
        Q_list, S_list, S_H_list, Pi_list = compute_stress_vector_step_variable_c1_relaxation(parameters,  datafile, sheet, p, load_step_values)
        fitted_stress_dict_relaxation[p] = Pi_list
        previous_step_values = Q_list[-1], S_list[-1], S_H_list[-1], Pi_list[-1]
        previous_step_optimized_parameters_relaxation = [optimized_tau_relaxation]
...
def plot_results_variable_c1(datafile, sheet, minimization_method_load, minimization_method_relaxation):
    index_init_step_dict_load, index_final_step_dict_load, elongation_init_step_dict_load, elongation_final_step_dict_load, elongation_list_during_steps_dict_load, time_list_during_steps_dict_load, stress_list_during_steps_dict_load, index_init_step_dict_relaxation, index_final_step_dict_relaxation, elongation_init_step_dict_relaxation, elongation_final_step_dict_relaxation, elongation_list_during_steps_dict_relaxation, time_list_during_steps_dict_relaxation, stress_list_during_steps_dict_relaxation = extract_step_data(datafile, sheet)    
    c1_dict_load, beta_dict_load, tau_dict_load, fitted_stress_dict_load, c1_dict_relaxation, beta_dict_relaxation, tau_dict_relaxation, fitted_stress_dict_relaxation = find_parameters_all_steps_variable_c1(datafile, sheet, minimization_method_load, minimization_method_relaxation)
    ...
    def plot_experimental_vs_fitted_data(datafile, sheet, minimization_method_load, minimization_method_relaxation):
        ...
        # savefigure.save_as_png(fig_elongation_vs_time, date + "_" + sheet + "_elongation_vs_time_exp_vs_model_load_relaxation_L_" + minimization_method_load + "_R_" + minimization_method_relaxation + "_variablec1")
        savefigure.save_as_png(fig_stress_vs_time, date + "_" + sheet + "_stress_vs_time_exp_vs_model_load_relaxation_L_" + minimization_method_load + "_R_" + minimization_method_relaxation + "_variablec1")
        savefigure.save_as_png(fig_stress_vs_elongation, date + "_" + sheet + "_stress_vs_elongation_exp_vs_model_load_relaxation_L_" + minimization_method_load + "_R_" + minimization_method_relaxation + "_variablec1")
    ...
    def plot_fitted_parameters(datafile, sheet, minimization_method_load, minimization_method_relaxation):
        ...
        savefigure.save_as_png(fig_c1_vs_step, datafile[0:6] + "_" + sheet + "fitted_params_c1_vs_load_step_L_" + minimization_method_load + "_R_" + minimization_method_relaxation + "_variablec1")
        savefigure.save_as_png(fig_beta_vs_step, datafile[0:6] + "_" + sheet + "fitted_params_beta_vs_load_step_L_" + minimization_method_load + "_R_" + minimization_method_relaxation + "_variablec1")
        savefigure.save_as_png(fig_tau_vs_step, datafile[0:6] + "_" + sheet + "fitted_params_tau_vs_load_step_L_" + minimization_method_load + "_R_" + minimization_method_relaxation + "_variablec1")

        savefigure.save_as_png(fig_c1_vs_elongation, datafile[0:6] + "_" + sheet + "fitted_params_c1_vs_load_elongation_L_" + minimization_method_load + "_R_" + minimization_method_relaxation + "_variablec1")
        savefigure.save_as_png(fig_beta_vs_elongation, datafile[0:6] + "_" + sheet + "fitted_params_beta_vs_load_elongation_L_" + minimization_method_load + "_R_" + minimization_method_relaxation + "_variablec1")
        savefigure.save_as_png(fig_tau_vs_elongation, datafile[0:6] + "_" + sheet + "fitted_params_tau_vs_load_elongation_L_" + minimization_method_load + "_R_" + minimization_method_relaxation + "_variablec1")
...
if __name__ == "__main__":
    ...
    # c1_dict_load, beta_dict_load, tau_dict_load, fitted_stress_dict_load = find_parameters_all_steps(datafile, sheet1)
        for minimization_method_load,minimization_method_relaxation  in list(itertools.product(minimization_method_load_list, minimization_method_relaxation_list)):
            try:
                plot_results(datafile, sheet, minimization_method_load, minimization_method_relaxation)
