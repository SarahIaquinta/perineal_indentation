...
from constitutive_laws import yeoh, mooney
plt.ion()
...
for c in range(len(all_datas)):
    ...
    for i, element in enumerate(datas):
        ...
        ax.plot(strainu, mooney(stretchu, *popt2), '-', color=col_cochon[i], alpha=0.95)
...
for i, fig in enumerate(list_of_figures) :
    ...
    fig.savefig(save_results_to + '/identification_mooney_' + fig_label + '_all_cochons_.jpg', dpi=300)
